...
function resolveDataSource() {
    ...
    return {
        ok: false,
        error: '无法定位 @lumoralab/ui：未找到已安装的 npm 包 (dist/componentSchema.json) 也未找到本地 monorepo (pnpm-workspace.yaml)。请安装 @lumoralab/ui 或设置 LUMORALAB_UI_REACT_ROOT 指向 packages/ui/react。',
    };
}
/**
 ...
 */
function tryLoadSchemaBundle(startDir) {
    try {
...
function getDataSource() {
    if (!cachedSource)
        cachedSource = resolveDataSource();
    return cachedSource;
}
// ---------------------------------------------------------------------------
// 工具结果辅助函数
...
function relFromRoot(reactRoot, absPath) {
    return path.relative(reactRoot, absPath).split(path.sep).join('/');
}
// ---------------------------------------------------------------------------
// source 模式：扫描磁盘
// ---------------------------------------------------------------------------
async function discoverComponentsFromFs(reactRoot) {
...
async function handleGetComponent(ds, name) {
    if (ds.mode === 'schema') {
    ...
    const source = found.entryFile ? await readFileSafe(found.entryFile) : null;
...
async function handleOverview(ds) {
    if (ds.mode === 'schema') {
...
server.registerTool('list_components', {
    ...
    description: '列出 @lumoralab/ui (React) 导出的所有组件，包含名称、类型（custom / re-export）、分类与描述。优先返回结构化 schema（来自已安装的 npm 包）。',
}, async () => {
    const ds = getDataSource();
    if (!ds.ok)
        return errorResult(ds.error);
    return okResult(await handleList(ds));
});
server.registerTool('get_component', {
    title: '读取组件详情',
    description: '根据组件名读取详情：schema 模式返回 props/示例/refMethods/类型定义；source 模式返回源码与类型文件路径。',
    inputSchema: {
        name: z
            .string()
            .min(1)
...
server.registerTool('get_library_overview', {
    ...
    description: '返回 @lumoralab/ui 的版本、peerDependencies、安装命令、import 示例、README。schema 模式还会带上 schema 生成时间与 bundle 路径。',
}, async () => {
    const ds = getDataSource();
    if (!ds.ok)
        return errorResult(ds.error);
...
if (!initial.ok) {
    ...
    console.error(`[@lumoralab/ui-mcp] 数据源：schema（${initial.bundle.components.length} 个组件，version=${initial.bundle.version}）`);
}
else {
